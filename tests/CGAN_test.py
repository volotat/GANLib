...
for i in range(1): #len(tests['dataset'])
    ...
    gan = GAN([X_train.shape[1:],labels.shape[1:]], noise_dim, distance = tests['distance'][i], n_critic = 3)
    ...
    gan.train([X_train, labels], epochs=5000, batch_size=64, checkpoint_callback = callback)
